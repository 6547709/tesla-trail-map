...
> Current version: **v1.6.5** · see the [CHANGELOG](./CHANGELOG.md)
...
```bash
docker pull ghcr.io/6547709/tesla-trail-map:1.6.5
...
docker run -d --name tesla-trail-map -p 8080:8080 \
  ...
  ghcr.io/6547709/tesla-trail-map:1.6.5
```
...
```bash
docker build -t tesla-trail-map:1.6.5 .
docker run -d --name tesla-trail-map -p 8080:8080 \
  -e DATABASE_HOST=host.docker.internal \
  -e DATABASE_PORT=5432 \
  -e DATABASE_USER=teslamate \
...
docker buildx build --platform linux/amd64,linux/arm64 \
  --build-arg VERSION=v1.6.5 -t tesla-trail-map:1.6.5 --load .
```
...
services:
  tesla-trail-map:
    image: ghcr.io/6547709/tesla-trail-map:1.6.5
